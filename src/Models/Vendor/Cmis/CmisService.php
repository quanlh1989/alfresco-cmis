...
// -- Change log token is an anomoly -- but included in URL as parameter
	 ...
	 * @since CMIS-1.0
	 */
	public function getTypeDescendants($typeId = null, $depth = 2, $options = array ()) {
...
<?php
	...
	public function getContentChanges($options = array()) {
        if(!isset($this->workspace->links[self::LINK_CHANGES])){
            return false;
        }
        $makeLink = $this->workspace->links[self::LINK_CHANGES]."?repositoryId=-default-&changeLogToken={changeLogToken}&includeProperties={includeProperties}";
		$myURL =  CmisRepositoryWrapper :: processTemplate($makeLink, $options);
